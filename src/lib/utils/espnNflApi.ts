...
export interface NflPlayerGroup {
  id: string
  name: string
  playerIds: string[]
  players: NflPlayer[]
...
function parseOdds(comp: any): NflGameOdds | null {
  ...
  let moneyLine: NflGameOdds['moneyLine'] = null
...
function parseGame(event: any): NflGame {
...
export async function fetchNflScoreboard(): Promise<NflGame[]> {
...
export async function fetchNflGameSummary(gameId: string): Promise<any> {
  const res = await fetch(`${BASE}/summary?event=${gameId}`)
  return res.json()
}

export function extractNflPlayerStats(summary: any, playerId: string): Record<string, string> | null {
...
const GROUPS_KEY = 'nfl-command-center-groups'

export function loadNflGroups(): NflPlayerGroup[] {
  try {
    return JSON.parse(localStorage.getItem(GROUPS_KEY) || '[]')
  } catch { return [] }
}

export function saveNflGroups(groups: NflPlayerGroup[]) {
  localStorage.setItem(GROUPS_KEY, JSON.stringify(groups))
}

export function isNflGameLive(game: NflGame): boolean {
  const name = game.status.type.name
  return name === 'STATUS_IN_PROGRESS' || name === 'STATUS_HALFTIME'
}

export function getNflGameStatusText(game: NflGame): string {
  if (isNflGameLive(game)) {
...
export function findNflPlayerGames(games: NflGame[], player: NflPlayer): NflGame[] {
